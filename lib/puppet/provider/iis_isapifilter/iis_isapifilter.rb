...
Puppet::Type.type(:iis_isapifilter).provide(:iis_isapifilter) do
	...
	def self.get_isapifilters
		output = powershell("Get-WebConfiguration -Recurse -Filter /system.webServer/isapiFilters/filter | select name,enableCache,enabled,path,preCondition,Location,PSPath | ConvertTo-Json")
