...
Puppet::Type.type(:iis_httperror).provide(:iis_httperror, parent: Puppet::Provider::IIS_PowerShell) do
  ...
  def self.get_httperrors
    cmd = "Get-WebConfiguration -Recurse -Filter /system.webServer/httpErrors/error | select statusCode,subStatusCode,prefixLanguageFilePath,path,responseMode,Location,PSPath"
