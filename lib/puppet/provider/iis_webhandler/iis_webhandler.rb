...
Puppet::Type.type(:iis_webhandler).provide(:iis_webhandler) do
	...
	def self.get_webhandlers
		output = powershell("Get-WebConfiguration -Recurse -Filter /system.webServer/handlers/add | select Name,Path,Verb,Type,Modules,ScriptProcessor,Precondition,ResourceType,RequireAccess,Location,PSPath | ConvertTo-Json")
		json = JSON.parse(output)
		json.map {|hash| hash.inject({}){|memo,(k,v)| memo[k.downcase.to_sym] = v; memo}}
	end
